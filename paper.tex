...
\Procedure{auto-involutive-mcmc}{$\mathcal{P}$, $\mathcal{Q}$, $\mathcal{F}$, $x$}
    ...
    \State\vspace{-2mm}
    \LineComment{{\small Compute $(x' \oplus y') = f(x \oplus y)$ and log(Radon-Nikodym derivative) via differentiable program $\mathcal{F}$}}
    \State $(x' \oplus y', \log D) \gets \textproc{run-involution}(\mathcal{F}, x \oplus y)$
    \State\vspace{-2mm}
...
@write(model_out[:polar], !polar, :discrete)
\end{lstlisting}}
\end{tabular}
\end{center}
...
Note that in general there are observations $b$ that induce the target density $p(x)$ via $p(x) \propto \tilde{p}(x \oplus b)$ as described in Section~\ref{sec:dists-on-dicts}, and the involution $f$ in general depends on $b$.
The involution program $\mathcal{F}$ can read the values of the $b$, which are stored alongside the unobserved random choices in the input model trace.
