...
The probabilistic program still defines a probability distibution (i.e. measure) on traces, but viavia  a density instead of a probability mass function.
...
Address $\mathtt{:}\mathtt{:noise}$ has $V_a = \reals_{> 0}$ and $(\mathtt{"c"}, i)$ and $(\mathtt{"y"}, i)$ have $V_a = \reals$, and all of these use the Lebesgue measure. % TODO double-check this
...
@tracefn h from (tr1, tr2) to (tr3, tr4)
   ...
   if is_birth
     ...
     @write(tr3, :k, k-1, :discrete)
     new_weights = delete_weight(weights, i)
   end
