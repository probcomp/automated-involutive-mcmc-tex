...
Many involutive MCMC kernels update only a constant number of keys that does not depend on the sizes $N$ and $N'$ of the input and output traces or the total number of continous keys $M$; the other keys are copied over unchanged.
...
We replace the separate calls to \textproc{score} for the log-densities $\log p(x)$ and $\log p(x')$ and replace them with a single call to \emph{trace-update} on $\mathcal{P}$ that takes in $x$, and $\delta$ and returns $x'$ and $\log p(x') - \log p(x)$.
When $|K_{\delta}|$ and $N_2$ and $N_2'$ are all constant in the size $N_1$ of the model trace $x$, the resulting algorithm uses a number of operations that is constant in $O(N_1 + N_1')$.
...
To test the user's algorithm, Gen first samples a model trace $x$ and simulated observations $b$ from the model prior $p$, and auxiliary trace $y$ from the $q_x$. It then runs three tests:
...
\item \textbf{Dimension check.} The \textit{dimension check} runs the involution $f$ on $(x, y)$ and records the sets of addresses $R$ and $W$ at which $f$ performs continuous reads and writes, respectively. It then checks that the sizes of these sets match.
This check is run when computing the determinant of the Jacobian in \textproc{run-involution}.
...
\end{tabular}
\end{center}
Note that the subtrees here happen to be trees that themselves have a random number of random choices, but the same general construction for a mixture of proposals below can also be used for simpler Metropolis-Hastings kernels:
\begin{enumerate}
\item The auxiliary probabilistic program samples from a distribution over different sets of random choices that will be proposed to
(in this case, each set is a different subtree of the parse tree).
\item The auxiliary probabilistic program then samples new values for those random choices (in this case, a new subtree).
\item The involution program swaps the previous values of those random choices with their new values, by swapping data between the model trace and the auxiliary trace.
\item The involution program copies the choice of what subset of random choices to propose to from the input auxiliary trace to the output auxiliary trace.
\end{enumerate}

Because the mixture distribution is specified using a probabilistic program, it is straightforward to modify the program to define a different mixture distribution.
...
Therefore, the mixture probabilities must be accounted for in the acceptance probability calculation.
...
For this alternative mixture distribution, the ratio of mixture probabilities is the ratio of sizes of the two trees.
