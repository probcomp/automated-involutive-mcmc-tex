...
Tsome herefore, the number of operations (which is dominated by the determinant) reduces from $M^3$ to $(M-|C|)^3$ where $|C|$ is the number of input keys that were copied to some output input key.
...
We now show how to modify the program $\mathcal{F}$ and its interpreter, so that both computing $z' = (x', y')$ from $z = (x, y)$, and computing the log density ratio $\log(p(x') / p(x))$, can take $O(1)$ operations (that is, constant in $N_1 + N_1'$) whn the involution $f$ has sparse structure and the model density $p$ has conditional independencies.
...
While the naive implementation requires $O(|K_x| + |K_{x'}|)$ operations, it is possible for more sophisticated implementations of \emph{trace-update} to run in $O(|K_{\delta}|)$ operations for certain probabilistic programs $\mathcal{P}$ by exploiting conditional independence structure program that leads to cancellations in the density ratio $p(x') / p(x)$.
...
Instead of explicitly specifying all of $z'$, it only needs to specify a subset of the trace $\delta$.
Therefore, the involution program can run in $O(|K_{\delta}| + |K_{y}| + |K_{y'}|)$ operations.
We also remove the two calls to \textproc{score} on $x$ and $x'$, and replace them with a single call to \emph{trace-update} on $\mathcal{P}$ that takes in $x$, and $\delta$ and returns $x'$ and $\log p(x') - \log p(x)$.
When $|K_{\delta}|$ and $|K_{y}|$ and $|K_{y}|$ are all constant in the size $N_1$ of the model trace $x$, the resulting algorithm scales as $O(1)$ instead of $O(N + N')$.
