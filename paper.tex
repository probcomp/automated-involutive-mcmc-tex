...
\Procedure{auto-involutive-mcmc}{$\mathcal{P}$, $\mathcal{Q}$, $\mathcal{F}$, $x$}
    ...
    \State $\log \tilde{p}(x \oplus b) \gets \textproc{score}(\mathcal{P}, x \oplus b)$ \Comment{Compute log-density via probabilistic program}
    \State $\log \tilde{p}(x' \oplus b) \gets \textproc{score}(\mathcal{P}, x' \oplus b)$ \Comment{Compute log-density via probabilistic program}
    \State $\log q_{x'}(y') \gets \textproc{score}(\mathcal{Q}_{x'}, y')$ \Comment{Compute log-density via probabilistic program}
    \State $\alpha \gets \min\{1, \exp(\log \tilde{p}(x' \oplus b) - \log \tilde{p}(x \oplus b) + \log q_{x'}(y') - \log q_x(y) + \log D)\}$ \Comment{Acceptance probability}
