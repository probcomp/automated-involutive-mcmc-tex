...
\citet{green1995reversible} uses a state space that is the countable union of `models', where for each model there is a Eucliean space of parameters.
Dictionaries possesses substantially more structure:
Parameters have keys, so it is possible to express that parameters are shared across different structures.
Instead of a monolithic `model', dictionaries use a more elaborate discrete state that includes the set of keys and the assignment to the discrete keys.
The additional structure of dictionaries enables the automation technique Section~\ref{sec:automating}.
...
The conditional density $p(d | b)$ is nonzero only if $A_d = \{\texttt{k}\} \cup \{\texttt{m}_i \mid 1 \leq i \leq k\} \cup \{\texttt{s}_i \mid 1 \leq i \leq k\}$ for some $k$ ($d$ does not contain y-values), and the denominator in the definition of $p(d | b)$ simplifies to the familiar sum of marginal likelihoods over all $k$, where each marginal likelihood is an integral over $\mathbb{R}^{2k}$.
