...
\Procedure{auto-involutive-mcmc}{$\mathcal{P}$, $\mathcal{Q}$, $\mathcal{F}$, $x$, $b$}
    ...
    \State $(x' \oplus y', \log D) \gets \textproc{run-involution}(\mathcal{F}_b, x \oplus y)$
    \State\vspace{-2mm}
...
\end{minipage}\\
\vspace{1mm}
\Procedure{run-involution}{$\mathcal{F}_b$, $z$}
...
Section~\ref{sec:sparsity} describes a more efficient approach that exploits sparsity in the involution (and the resulting cancellations in the acceptance ratio) for improved efficiency, but requires a more sophisticated probabilistic programming runtime system.
...
This section shows that if the involution $f$ is specified in using a differentiable program $\mathcal{F}$ that transforms the execution traces of probabilistic programs, then the Jacobian factor in Equation~(\ref{eq:acceptance-ratio-dictionaries}) can also be automated, using automatic differentiation.
...
For example, to read the value of a continuous address $\mathtt{:}\mathtt{r}$ from the input model trace ($x$):
\noindent
\begin{center}
\begin{tabular}{c}
{\begin{lstlisting}[basicstyle=\small\ttfamily]
    val = @read(model_in[:r], :continuous)
\end{lstlisting}}
\end{tabular}
\end{center}
...
\begin{tabular}{c}
{\begin{lstlisting}[basicstyle=\small\ttfamily]
    @write(model_in[:r], val, :continuous)
\end{lstlisting}}
\end{tabular}
\end{center}
...
\begin{tabular}{c}
{\begin{lstlisting}[basicstyle=\small\ttfamily]
    @copy(model_in[:u], model_in[:v])
\end{lstlisting}}
\end{tabular}
\end{center}
Of course, it is also possible to copy from $x$ to $y'$, from $y$ to $x'$ and from $y$ to $y'$.
As we will see in Section~\ref{sec:sparsity}, it is preferable to use $\mathtt{@copy}$ when possible instead of reading and then writing, as this can improve the efficiency of automatic differentiation.
...
\end{tabular}
\end{center}
This is a simplified version of the unfinite Gaussian mixture model in Figure~\ref{fig:mixture}b.
Each key of the form $(\mathtt{:}\mathtt{x}, i)$ will be observed (in the dictionary $b$), and the latent part of the trace (the dictionary $x$) contains the other keys $\mathtt{:}\mathtt{k}$ and $(\mathtt{:}\mathtt{mu}, 1)$ and (only if $\mathtt{k} = 2$) $(\mathtt{:}\mathtt{mu}, 2)$.
...
We extend the involution using a pair of bijections between $(\mu_1, \mu_2)$ and $(\mu_1, u)$:
\[
\mu_1, \mu_2 \mapsto ((\mu_1 + \mu_2) / 2, \mu_2 - (\mu_1 + \mu_2)/2)
\]
\[
\mu_1, u \mapsto (\mu_1 - u, \mu_1 + u)
\]
...
To denote this dependency we use notation $\mathcal{F}_b$.
...
Therefore, the number of operations (which is dominated by the determinant) reduces from $M^3$ to $(M-|C|)^3$ where $|C|$ is the number of input keys that were copied to some output input key.
...
Then, the previous trace $x$ and a dictionary $\delta$ with $K_{\delta} = \Delta$ and $\delta[k] = x'[k]$ and the density function $p$ suffice to uniquely define the trace $x'$, provided the density satisfies the following condition, which is satisfied by densities as probabilistic programs:
...
(As a simple example, consider a density $p$ that factors into a product of independent densities, one for each random choice; and an involution that only modifies one of these choices).
...
Instead of explicitly specifying a value for all keys in $x'$, it can specify $x'$ by only explicitly specifying values in $\delta$.
Therefore, the involution program can run in $O(|K_{\delta}| + N_2 + N_2')$ operations.
We replace the separate calls to \textproc{score} for the log-densities $\log \tilde{p}(x \oplus b)$ and $\log \tilde{p}(x' \oplus b)$ and replace them with a single call to \emph{trace-update} on $\mathcal{P}$ that takes in $x$ and $\delta$ and returns $x'$ and $\log (\tilde{p}(x' \oplus b) / \tilde{p}(x \oplus b))$.
...
% TODO need to differentiate between number of data and latent dimension here!
% it is only constant in the latent dimension, not in the data.. (except in unusual cases)
...
\Procedure{involutive-mcmc-check}{$\mathcal{P}$, $\mathcal{Q}$, $\mathcal{F}$}
    ...
    \State $(x \oplus b) \sim \textproc{trace-and-score}(\mathcal{P})$ % TODO not conditioned on data?
    \State $y \sim \textproc{trace-and-score}(\mathcal{Q}_x)$
    ...
    \State $x' \oplus y' \gets \textproc{run-involution}(\mathcal{F}, x \oplus y)$
    ...
    \State $\tilde{x} \oplus \tilde{y} \gets \textproc{run-involution}(\mathcal{F}, x' \oplus y')$
...
At each iteration of the MCMC algorithm, the auxiliary probabilistic program $\mathcal{Q}$ (Figure~\ref{fig:structure-learning}d) first picks a random node in the parse tree of the covariance function, by doing a stochastic walk of the existing parse tree that terminates at the chosen node.
\begin{center}
\begin{tabular}{c}
{\begin{lstlisting}[basicstyle=\small\ttfamily]
...
In particular, the rest of the auxiliary probabilistic program $\mathcal{Q}$ proposes a new subtree by sampling from the same process used to recursively define the prior distribution:
\begin{center}
\begin{tabular}{c}
{\begin{lstlisting}[basicstyle=\small\ttfamily]
...
Note that because the mixture distribution is specified using a probabilistic program, it is straightforward to modify the program $\mathcal{Q}$ to define a different mixture distribution.
