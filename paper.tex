...
\textbf{Measure space for finite dictionaries.} We fix a countably infinite set $\mathcal{A}$ of possible keys, and for each key $a \in \mathcal{A}$,
we fix a measure space $(V_a, \Sigma_a, \mu_a)$ determining the possible values $V_a$ that may be
associated with $a$. Given a set of keys $A$, we can consider the set $V_A = \times_{a \in A} V_a$ of \textit{assignments} of values to each key. ($V_A$ can also be made a measure space, using the standard product $\sigma$-algebra $\Sigma_A = \otimes_{a \in A} \Sigma_a$ and the product measure $\mu_A = \times_{a \in A} \mu_a$.)  Then the set of all finite dictionaries is $\mathcal{D} := \bigcup_{A \subset \mathcal{A}, |A| < \infty} \{(A, \mathbf{x}) \mid \mathbf{x} \in V_A\}$. That is, a dictionary specifies 
...
of dictionaries. A reference measure $\mu_\mathcal{D}$ on this space can be constructed using the product measures $\mu_A$: we set $\mu(B) := \sum_{A \in \mathcal{A}, |A| < \infty} \mu_A(\{\mathbf{x} \mid (A, \mathbf{x}) \in B\})$.

\textbf{Notation for dictionaries.} Given a dictionary $x = (A, \mathbf{x})$, we write $A_m$ for $A$ and $m[a]$ for the value $x_a$ associated with a key $a \in A$.

\textbf{Probability distributions on finite dictionaries.} 
When all keys are discrete (have countable $V_a$ and counting measure for $\mu_{a}$), a probability distribution on dictionaries is defined by a $p$ is a probability mass function $p : \mathcal{D} \to [0, 1]$ that assigns a probability $p(m)$ to each dictionary $m \in \mathcal{P}$ such that $\sum_{x \in \mathcal{D}} p(m) = 1$.
...
\textit{Gaussian mixture model with unknown number of components.} Consider a generative model of univariate data points $y_1, \dots, y_n$ from a Gaussian mixture with an unknown number of components $k$, each with unknown mean $m_i$ and standard deviation $s_i$. If we place a Gaussian prior on $m_i$, a Gamma prior on $s_i$, and a Poisson prior on $k$, the resulting density is
%
$$p(d) = TODO$$
%
...
Let $f : Z \to Z$ denote an involution ($f^{-1} = f$) such that the pushforward of $\mu$ under $f$, denoted $\mu \circ f^{-1}$, is absolutely continuous with respect to $\mu$, with Radon-Nikodym derivative $d (\mu \circ f^{-1}) / d\mu : Z \to [0, \infty)$. Involutive MCMC runs $f$ on $(x, y)$ to obtain $(x', y')$, then computes an acceptance probability:
\begin{equation}
\alpha(x, y) = \frac{p(x') q_{x'}(y')}{p(x) q_{x}(y)} \cdot \left( \frac{d (\mu \circ f^{-1})}{d \mu} (x, y)\right)
\end{equation}
...
The general measure-theoretic formulation of involutive MCMC above does not specify how to compute a Radon-Nikodym derivative, or the densities.
We now derive an algorithm for involutive MCMC when the model and auxiliary distributions are defined on spaces of dictionaries.

% TODO should we just segment the addresses into discrete and continuous addresses as part of the dictionary formalism above -- that would allow us to move this up. the added flexibility of full per-address measure is not used anyways..

Suppose that the model distribution $p$ and auxiliary distributions $q_x$ are all probability distributions on dictionaries.
Then, $X$ and $Y$ are both sets of dictionaries, and the joint space $Z$ is a set of pairs $(x, y)$ of dictionaries.
Suppose that both $X$ and $Y$ use the set of keys $\mathcal{A}$, and that $\mathcal{A}$ can be partitioned $\mathcal{A} = \mathcal{I} \cup \mathcal{K}$ where is a set of \emph{discrete keys} $\mathcal{I}$ that use countable $V_a$ and counting measure $\mu_a$ for $a \in \mathcal{I}$ and $\mathcal{K}$ is a set of \emph{continuous keys} that use $V_a = \mathbb{R}^{d_a}$ for some $d_a$ and Lebesgue-measure $\mu_a$ for $a \in \mathcal{K}$.
%And suppose that there exists an involution $f : Z \to Z$ such that (i) 
Suppose there is a countable partition of $Z$ into $\{Z_e : e \in E\}$ such that across each $Z_e$ the sets of keys $A_x$ and $A_y$ and the values $\mathbf{x}_{\mathrm{d}}$ and $\mathbf{y}_{\mathrm{d}}$ of discrete keys are both constant (so that each set $Z_e$ is isomorphic to a Euclidean space of assignments to the continuous keys in the two dictionaries).
...
Algorithm~\ref{alg:involutive-mcmc} defines involutive MCMC in this more structured setting, in which the Radon-Nikodym is replaced with a Jacobian (see Section~\ref{sec:radon-nikodym-special-case} for a derivation).
\begin{algorithm}[h]
\begin{algorithmic}
\Procedure{involutive-mcmc-kernel}{$p$, $q$, $f$, $x$}
    \State $y \sim q_x(\cdot)$ \Comment{Sample auxiliary dictionary}
    \State $(x', y') \gets f(x, y)$ \Comment{Apply involution}
    \State $\alpha \gets
        \displaystyle \frac{p(x') q_{x'}(y')}{p(x) q_{x}(y)} \cdot |J h_e|(z)$
    \State $r \sim \mathrm{Uniform}(0, 1)$
    \State \algorithmicif \, $r \le \alpha$ \algorithmicthen \, \Return $x'$ \algorithmicelse \, \Return $x$ 
\EndProcedure
\end{algorithmic}
\caption{Involutive MCMC for Dictionaries}
...
\textit{Probabilistic programs} are flexible and structured representations for arbitrary
computable probability distributions. Unlike densities (but like Bayesian networks), probabilistic programs can be efficiently sampled and contain explicitly represented information
...
This section introduces one language for probabilistic programs, with which users can encode both their
models $p$ and their auxiliary distributions $q_x$. 
...
The measure-theoretic formalism above is capable of expressing probabilistic programs that use continuous distributions by setting the reference measure space $(V_a, \Sigma_a, \mu_a)$ for some addresses $a$ to use the Lebesgue-measure.
...
\end{array}
\end{align*}
where $m(t, i) := \sum_{j=0}^{x[\mathtt{:}\mathtt{degree}]} \mathtt{xcoords}[i]^j \cdot x[(\mathtt{"c"}, j)]$.

\subsection{Specifying the Densities for Involutive MCMC with Probabilistic Programs}
...
\subsubsection{Conditional Distributions on Traces}
...
\section{DIFFERENTIABLE PROGRAMMING WITH TRACES} \label{sec:differentiable-programming-traces}
...
\subsection{A Differentiable Programming Language for Manipulating Traces}
...
\subsection{Reasoning About Involutions}
...
% TODO do a PyTorch implementation??

\section{AUTOMATIC ACCEPTANCE PROBABILITY CALCULATION} \label{sec:automating}

The general form of the acceptance probability in Algorithm~\ref{alg:involutive-mcmc} is:
\begin{equation}
\min\left\{1, \frac{p(x') q_{x'}(y')}{p(x) q_{x}(y)} \left( \frac{d (\mu \circ f^{-1})}{d \mu} (x, y)\right) \right\}
\end{equation}
For the setting when $p$, $q$, and $f$ are defined using the probabilistic and differentiable programming languages of this section, the probability simplifies to:
\begin{equation} \label{eq:accept-prob-jacobian}
\min\left\{1, \frac{p(x') q_{x'}(y')}{p(x) q_{x}(y)} |J f_{2,i(x,y)} | \right\}
\end{equation}
Implementing the computation of this probability by hand is error-prone due to conceptual, arithmetic, and implementation errors; as well as time consuming and tedious.
This section describes how the computation of this probability is automated by Gen using probabilistic and differentiable programming techniques.

\subsection{Automatically Computing Probability Density Ratios}

When the model density function $p$ and the auxiliary density functions $q_x$ in Algorithm~\ref{alg:involutive-mcmc} are specified by writing probabilistic programs in the language of Section~\ref{sec:prob-prog-traces}, Gen automatically computes the following ratio of densities, given the previous traces $x$ and $y$ and the proposed traces $x'$ and $y'$:
\begin{equation} \label{eq:density-ratio}
\frac{p(x') q_{x'}(y')}{p(x) q_x(y)}
\end{equation}
The density $p$ of a probabilistic program with respect to the reference measure $\mu$, evaluated at a trace $x = (A, \mathbf{x})$, is the product of individual densities for each random choice (and similarly for the auxiliary densities and the auxiliary probabilistic program):
\begin{equation} \label{eq:density-product}
p((A, \mathbf{x})) = \prod_{a \in A} p_{\sigma_a}(\mathbf{x}[a])
\end{equation}
where $\mathbf{x}[a]$ denotes the value of address $a$ in the trace, $p_{\sigma_a}$ is the probability density function for distribution from which random choice $a$ is sampled, given that the state of the program when sampling is $\sigma_a$.
(See~\citet{popltraces} for a formal proof for a somewhat more restricted probabilistic programming language.)
Therefore, the probabilistic programming language can compute the density at a trace $x$ by executing the program, but at each random choice $a$, reading the value of the choice $\mathbf{x}[a]$ instead of sampling it, computing the density at $\mathbf{x}[a]$ and accumulating it into the product above.\footnote{Probabilistic programming implementations use sums of log-densities instead of products for numerical reasons.}

To evaluate this density ratio, we utilize Gen's API for probabilistic programs and traces, which automatically computes the density of a trace of a probabilistic program, whether that trace was sampled from the probabilistic program (as in the case of computing $q_x(y)$ for $y \sim q_{x}(\cdot)$) or when that trace was produced externally from the probabilistic program (as in the case of computing $q_{x'}(y')$, where $y'$ was produced from the involution $f$).
