...
\end{tabular}
\end{center}
As we will see in Section~\ref{sec:sparsity}, it is preferable to copy when possible instead of reading and then writing, as the additional information can improve the efficiency of automatic differentiation.
...
Each input and output trace is given a name (e.g. \texttt{tr1}, \texttt{tr2}, \texttt{tr3}, and \texttt{tr4}).
For involution MCMC, the function must have two input traces and two output traces.
The first input trace is the input model trace ($x$), the second input trace is the input auxiliary trace ($y$), the first output trace is the output model trace ($x'$) and the second output trace is the output auxiliary trace ($y'$).
...
\end{tabular}
\end{center}
In order to apply this function in the context of involution MCMC, we require probabilistic programs $\mathcal{P}$ and $\mathcal{Q}$ that define space of input and output traces for the involution.
A minimal example does not require any auxiliary random choices, so $\mathcal{Q}$ can be an empty probabilistic program.
A minimal toy example of a probabilistic program $\mathcal{P}$ that is compatible with the involution \texttt{h} defined above is:
\noindent
\begin{center}
\begin{tabular}{c}
{\begin{lstlisting}[basicstyle=\small\ttfamily]
...
\subsubsection{Example: Split-Merge Reversible Jump}

Figure~\ref{fig:mixture}(d) shows an involution expressed in the differentiable programming language introduced in this section, and Figure~\ref{fig:mixture}(f) shows a schematic of the information flow through the involution as it is applied twice in a row.
The involution implements a split or a merge proposal in the infinite mixture model of Figure~\ref{fig:mixture}(b), depending on the trace of the auxiliary program in Figure~\ref{fig:mixture}(c).
If there are at least two clusters, and the value of address $\mathtt{split}$ is false, then the involution proposes to merge two clusters by running code block 2 shown in green.
Otherwise, the involution proposes to split a cluster by running code block 1 shown in yellow.
Note that that code block 1 (yellow) is itself an involution on the discrete addresses in the traces---each application of code block 1 to the traces causes them to flip back and forth between a split and a merge proposal.
Also note that code blocks 2 and 3 are continuously differentiable bijections on the continuous addresses in the traces, and they are inverses of one another.

\subsubsection{Computing the Jacobian with Automatic Differentiation}
Recall that the involution $f$ is composed of (i) an involution $g$ on elements $e$ of a partition of the state space, and (ii) a pair of continuous differentiable bijections $h_{e}$ and $h_{g(e)} = h_{e}^{-1}$ between each pair of corresponding elements of the partition.
...
After $\mathcal{F}$ is finished executing, AD is used to compute the Jacobian $|J h_e|(z)$; in the case of reverse-mode AD by iterating over output continuous addresses and backpropagating from each one to all of the input continous addresses, computing the Jacobian row-by-row.
The address metadata that was recorded during the first pass through the function is used to make AD more efficient, as discussed in the next section.
...
To represent reversible jump MCMC using involutive MCMC implemented with probabilistic programs, we write a probabilistic program $\mathcal{P}$ that encodes the space of models $\mathcal{K}$, the prior distribution on models, $p(k)$, the per-model priors $p_k(\theta_k)$ and the per-model likelihoods $L_{D,k}(\theta_k)$.
The set of all models $k$ corresponds to the set of all pairs $(A_P, \mathbf{d}_P)$ where $A_\mathcal{P}$ represent possible trace structures (i.e. control-flow paths through $\mathcal{P}$) and $\mathbf{d}_P$ are the set of assignments to discrete random choices made by $\mathcal{P}$.
