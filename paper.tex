...
\section{INVOLUTIVE MCMC ON GENERAL STATE SPACES}
...
\textbf{Notation for dictionaries.} Given a dictionary $m = (A, \mathbf{x})$, we write $A_m$ for $A$ and $m[a]$ for the value $x_a$ associated with a key $a \in A$.

\textbf{Densities on finite dictionaries.} Probability distributions on dictionaries can be defined by (i) choosing 
domains $V_a$ for each address $a$ of interest, and (ii) choosing a probability density $p : \mathcal{D} \rightarrow [0, \infty)$ such that $\int p(m) \mu_\mathcal{D}(\text{d}m) = 1$. We now give several examples
to build intuition.
...
Let $f : Z \to Z$ denote an involution ($f^{-1} = f$) such that the pushforward of $\mu$ under $f$, denoted $\mu \circ f^{-1}$, is absolutely continuous with respect to $\mu$, with Radon-Nikodym derivative $d (\mu \circ f^{-1}) / d\mu : Z \to [0, \infty)$. Involutive MCMC runs $f$ on $(x, y)$ to obtain $(x', y')$, then computes an acceptance probability $\alpha$. With probability $\alpha$, the new state $x'$ is returned; otherwise, the previous state $x$ is repeated.
%Let $y \sim q_x(\cdot)$ denote a random sample from the measure defined by $A \mapsto \int_A q_x(y) \mu_Q(dy)$ for $A \in \Sigma_Q$.

% TODO what is a trace? and what is a p? do we need parents?
%\begin{algorithm}[h]
%\begin{algorithmic}
%\Procedure{trimcmc-kernel}{$p$, $q$, $f$, $x$}
    %\State $y \sim q_x(\cdot)$ \Comment{Sample auxiliary trace}
    %\State $(x', y') \gets f(x, y)$ \Comment{Apply involution}
    %\State $w_1 \gets \prod_{a \in A(x)} p_{A(x)}(x_a | x_{\mathrm{Pa}^{A(x)}_a})$
    %\State $w_2 \gets \prod_{a \in A(y)} q_{x,A(y)}(y_a | x_{\mathrm{Pa}^{A(x)}_a})$
    %\State $w_3 \gets \prod_{a \in A(x')} p_{A(x')}(x'_a | (x'_b)_{b \in \mathrm{Pa}^{A(x)}_a})$
    %\State $w_4 \gets \prod_{a \in A(y')} q_{x',A(y')}(y'_b | (y'_b)_{b \in \mathrm{Pa}^{A(x)}_a})$
    %\State $|J| \gets F.\textproc{jacdetabs}(x, y)$
    %\State $\alpha \gets
        %\displaystyle \frac{w_3 \cdot w_4}{w_1 \cdot w_2} \cdot w_5$
    %\State $r \sim \mathrm{Uniform}(0, 1)$
    %\State \algorithmicif \, $r \le \alpha$ \algorithmicthen \, \Return $x'$ \algorithmicelse \, \Return $x$ 
%\EndProcedure
%
%\Procedure{$P$.score}{$x$}
    %%\State \Return $p(x)$ % TODO or product of densities? % TODO state notation is non-standard
    %% if we don't use this product then what is the point -- why don't we just use p(x)
    %\State \Return $\prod_{a \in A(x)} p_{\sigma(a)}(x[a])$ (?) TODO % TODO
%\EndProcedure
%
%\Procedure{$Q$.score}{$y$; $x$}
    %%\State \Return $q_x(y)$ % TODO or product of densities?
    %\State \Return $\prod_{a \in A(y)} q_{x,\sigma(a)}(y[a])$ (?) TODO % TODO
%\EndProcedure
%
%\Procedure{$F$.jacdetabs}{$x$,$y$}
    %\For{$a$ in $\textproc{continuous-addresses}(x)$}
        %\State $J1[a,:] \gets \nabla_{x[a]} \left( F(x, y)_{\mathrm{continuous}} \right)$ (?) TODO % TODO
    %\EndFor
    %\For{$a$ in $\textproc{continuous-addresses}(y)$}
        %\State $J2[a,:] \gets \nabla_{y[a]} \left( F(x, y)_{\mathrm{continuous}} \right)$ (?) TODO % TODO
    %\EndFor
    %\State $J \gets \left[\begin{array}{c}J1\\J2\end{array}\right]$
    %\State \Return $|\det(J)|$
%\EndProcedure
%\end{algorithmic}
%\caption{Trace Involutive MCMC}
%\label{alg:trimcmc}
%\end{algorithm}
...
See Section~\ref{sec:radon-nikodym-special-case} for a derivation of the Radon-Nikodym derivative $d (\mu \circ f^{-1}) / d \mu$ for a special case, when the involution $f$ can be factored into (i) an involution on a countable partition of $Z$ and (ii) a family of continuously differentiable bijections on Euclidean spaces.
