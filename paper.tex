...
Suppose $N$ and $N'$ are the total number of random choices in the input traces $x, y$ and output traces $x', y'$ respectively, and $M$ is the number of continuous random choices (which must be the same in the input traces and output traces).
...
Consider input traces $(x, y)$ and output traces $(x', y')$ with $f(x, y) = (x', y')$ for involution $f$ encoded by program $\mathcal{F}$.
Let $N_1 := |K_{x}|$, $N_2 := |K_{y}|$, $N_1' := |K_{x'}|$ and $N_2' := |K_{y'}|$, and
$N := N_1 + N_2$ and $N' := N_1' + N_2'$.
Suppose that $N_1 + N_1' \gg N_2 + N_2'$, which occurs when the size of the model traces grows in the size of a data set, but the size of the auxiliary trace is constant.
...
When $|K_{\delta}|$ and $N_2$ and $N_2'$ are all constant in the size $N_1$ of the model trace $x$, the resulting algorithm uses a number of operations that does not grow with $N_1 + N_1'$.
