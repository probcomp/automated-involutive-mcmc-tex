...
In recent decades, a number of probabilistic programming systems have provided automated implementations of probabilistic inference algorithms~\citep{gilks1994language,milch2007,pfeffer200714,goodman2012church,gehr2016psi,carpenter2017stan}.
The majority of these systems have focused on the automation of one or more built-in inference algorithms, with customization limited to tuning a handful of parameters of the algorithm.
The programmable inference paradigm~\citep{mansinghka2018probabilistic} in probabilistic programming instead emphasizes user control over the inference algorithm and automation of the lower-level implementation.
In particular, the Gen probabilistic programming system~\citep{cusumano2019gen} offers a high degree of user programmability by exposing an API that supports high-level user implementations of inference algorithms, instead of a set of a built-in inference engine.
One other probabilistic programming system besides Gen supports custom reversible jump samplers:
\citet{roberts2019} present a system embedded in Haskell that can automatically generate the implementation of certain reversible jump MCMC kernels from a high-level specification, and \citet{narayanan2020symbolic} also give a technique that can be used to automatically compute the Metropolis-Hastings acceptance probabilities in some settings; however, these approaches do not handle many kernels that can be handled by our technique, including the example in Figure~\ref{fig:structure-learning}.
...
Then, the number of operations used in Algorithm~\ref{alg:auto-involutive-mcmc} grows as $O(N + N') + O(M^3)$ (where each call to \textproc{logpdf} and the computation of each entry in the Jacobian both count as one operation).
...
The cubic term represents the complexity of computing the Jacobian matrix and its determinant.
...
Indeed, if some input variable is copied to some output variable, then we can entirely avoid computing its row (and corresponding column) of the Jacobian, and the computational complexity reduces from $M^3$ to $(M-K)^3$ where $K$ is the number of input variables that were copied from some input variable.
...
Many involutive MCMC kernels update only a constant number of variables that does not depend on the sizes $N$ and $N'$ of the input and output traces or the total number of continous keys $M$; the other variables are copied over unchanged.
...
Therefore, the computation complexity of the acceptance ratio computation reduces from $O(N + N') + O(M^3)$ to $O(N + N')$.
...
It is possible to further reduce the the number of operations in automated involutive MCMC from $O(N + N')$ to $O(N_2 + N_2')$ where $N = N_1 + N_2$ and $N' = N_1' + N_2'$ and $N_1$ and $N_2$ are the sizes of the model trace and auxiliary trace respectively (and similarly for $N_1'$ and $N_2'$).
In cases where $N_2$ and $N_2'$ have fixed size even as $N_1$ and $N_1'$ grow, this approach can result in $O(1)$ operations per kernel application.
The efficiency gains are due to a combination of exploiting sparsity in the involution and conditional independencies in the model.

Consider input traces $(x, y)$ and output traces $(x', y')$ where $x$ and $x'$ have sizes $N_1$ and $N_2$ respectively.
The approach in Algorithm~\ref{alg:auto-involutive-mcmc} involves running a program $\mathcal{F}$ that explicitly writes the value for each element of $(x', y')$, which requires greater than $N_1'$ `write' or `copy' operations.
We now show how to specify the involution $f$ such that computing $z'$ from $z$ can take fewer operations for involutions that possess sparse structure.
Recall that $K_{x}$ and $K_{x'}$ are the sets of keys in $x$ and $x'$, respectively, and that $x$ and $x'$ may both have values for certain keys, and $x$ may contains keys not present in $x'$, and vice-versa.
...
Then, the previous trace $x$ and a dictionary $\delta$ with $K_{\delta} = \Delta$ and $\delta[k] = x'[k]$ and the density function $p$ suffice to uniquely define the trace $x'$, provided the density satisfies the following condition:
...
Then, \textproc{naive-trace-update} (Algorithm~\ref{alg:naive-update}) computes the new trace $x'$ and difference in log-densities $\log p(x') - \log p(x)$ of a probabilistic program from a previous trace $x$ and a dictionary $\delta$, provided there exists $x' = \delta \oplus x|_A$ for some $A$ where $p(x') > 0$.
\begin{algorithm}[h]
\begin{algorithmic}
...
Involutive MCMC is a unifying construction for MCMC algorithms that encompasses both classic approaches to constructing kernels like reversible jump MCMC~\citep{green1995reversible}, but also recently introduced classes of MCMC kernels based on neural networks~\citep{spanbauer2020deep}.
