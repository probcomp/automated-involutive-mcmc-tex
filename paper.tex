...
Then, the number of operations requried in Algorithm~\ref{alg:auto-involutive-mcmc} grows as $O(N + N') + O(M^2)$ (where each call to \textproc{logpdf} and the computation of each entry in the Jacobian both count as one operation).
...
Therefore, the computation complexity of the acceptance ratio computation reduces from $O(N + N') + O(M^2)$ to $O(N + N')$.
