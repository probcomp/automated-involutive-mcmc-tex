...
\Procedure{trace-and-score}{$\mathcal{P}$}
   ...
   \execute[$\mathcal{P}$], but with $\mathtt{"}a \sim \mathrm{distribution}\mathtt{"} \equiv$ (
        ...
        \State 3. set $\log w \gets \log w + \textproc{logpdf}(\mathrm{distribution}, v)$
...
\Procedure{score}{$\mathcal{P}$, $x$}
   ...
   \execute[$\mathcal{P}$], but with $\mathtt{"}a \sim \mathrm{distribution}\mathtt{"} \equiv$ (
        ...
        \State 2. set $\log w \gets \log w + \textproc{logpdf}(\mathrm{distribution}, v)$
        \State 3. evaluate to $v$)
    \END
   \State \Return $\log w$
\EndProcedure
\end{minipage}\\
