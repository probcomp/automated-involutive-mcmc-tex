...
% TODO would be much simpler to describe if we didn't have to separate out x and y..
    ...
    \execute[$\mathcal{F}$], but with
        \State $\mathtt{"write\_disc}(a, v)\mathtt{"} \equiv$ (set $z'[a] \gets v$)
        \State $\mathtt{"read\_disc}(a)\mathtt{"} \equiv$ (evaluate to $z[a]$)
        \State $\mathtt{"write\_cont}(a, v)\mathtt{"} \equiv$ (1. set $z'[a] \gets v$ and 2. set $W \gets W \cup \{a\}$)
        \State $\mathtt{"read\_cont}(a)\mathtt{"} \equiv$ (1. set $R \gets R \cup \{a\}$ and 2. evaluate to $z[a]$)
        \State $\mathtt{"copy}(a_1, a_2)\mathtt{"} \equiv$ (1. set $z'[a_2] \gets z[a_1]$ and 2. set $C \gets C \cup \{a_2\}$)
    \END
    ...
    \For{$i$ in $1, \ldots, |W \setminus C|$}
        \State $a_i \gets (W \setminus C)_i$ \Comment{Pick $i$th key in $(W \setminus C)$; the order does not matter}
        \State $J[i,:] \gets \nabla_{\mathbf{z}_{R \setminus C}} \left( h_{e(z)}[a_i] \right)$ \Comment{{\small Gradient of $z'[a_i] = h_{e(z)}[a_i]$ wrt non-copied continuous inputs ($z_a$ for $a \in R \setminus C$)}}
    \EndFor
