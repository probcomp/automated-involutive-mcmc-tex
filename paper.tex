...
Let $p : X \to [0, \infty)$ be a probability density with respect to $\mu_P$ and for each $t \in X$ such that $p(t) > 0$ let $q_t : Y \to [0, \infty)$ be probability density with respect to $\mu_Q$.
We call $p$ the \emph{target density} and we call $q_t$ \emph{auxiliary densities}.
Let $\pi(t, u) := p(t) q_t(u)$ and $Z := \{(t, u) : \pi(t, u) > 0 \}$.
Suppose $Z \in \Sigma_P \otimes \Sigma_Q$ (the product $\sigma$-algebra), so that $\Sigma := \{A \in \Sigma_P \otimes \Sigma_Q : A \subseteq Z\}$ is a $\sigma$-algebra on $Z$ and let $\mu : \Sigma \to [0, \infty)$ with $\mu(A) := (\mu_P \times \mu_Q)(A)$
...
Let $u \sim q_t(\cdot)$ denote a random sample from the measure defined by $A \mapsto \int_A q_t(u) \mu_Q(du)$ for $A \in \Sigma_Q$.

\begin{algorithm}[h]
\begin{algorithmic}
\Procedure{involutive-mcmc-move}{$p$, $q$, $f$, $t$}
    \State $u \sim q_t(\cdot)$ \Comment{Sample auxiliary variables}
...
pecifically, we define the $\mathcal{S}$ to be the set of all possible \emph{trace structures} as a (possibly countably infinite) set of finite sets of addresses, denoted $\mathcal{S} \subseteq \mathcal{P}(\mathcal{A})$, where $\mathcal{A}$ is the set of all addresses.
For each address $a \in \mathcal{A}$ we assign a reference measure space $(X_a, \Sigma_a, \mu_a)$ where $\mu_a$ is $\sigma$-finite.
The set of all traces of a probabilistic program is the set $T := \cup_{S \in \mathcal{S}} \{(S, \mathbf{x}) : \mathbf{x} \in \times_{a \in S} X_a\}\}$.
The \emph{reference measure space} for a probabilistic program is $(T, \Sigma, \mu)$ where
$\Sigma$ is the $\sigma$-algebra that contains sets $\cup_{S \in \mathcal{S}}\{ B_S \} \in \mathcal{P}(T)$ such that $B_S$ is an element of the product $\sigma$-algebra $\Sigma_S := \otimes_{a \in S} \Sigma_a$,
and where $\mu(\cup_{S \in \mathcal{S}}\{ B_S \}) := \sum_{S \in \mathcal{S}} \mu_S(B_S)$ where $\mu_S$ is the product measure $\times_{a \in S} \mu_a$.

We assume that a probabilistic program $P$ denotes a reference measure space $(T_P, \Sigma_P, \mu_P)$ and a measure $P(dt)$ that is mutually absolutely continuous with respect to $\mu_P$, with density 
$p : T \to [0, \infty)$ on traces $t \in T$.
...
A second probabilistic program that is parametrized by (i.e. takes as input) traces of the first probabilistic program $P$.
The reference measure space of the second probabilistic program for $t \in T_P$ is denoted $(T_{Q,t}, \Sigma_{Q,t}, \mu_{P,t})$.
We define $Z := \{(t, u) : t \in T_P, u \in T_{Q,t}$ and we define $\pi(z) := p(t) q_t(u)$.
We require that $(Z, \Sigma, \mu)$ is a probabilistic program reference measure and $\pi$ is a probability density with respect to $\mu$ that is mutually absolutely continuous.
The combined program is $P \circ Q$.
