...
Involutive MCMC constructs an MCMC kernel from three components: (i) the unnormalized target density, (ii) a sampler and density for an auxiliary probability distribution, and (iii) an involution\footnote{An \emph{involution} is a bijection that is its own inverse (that is, $f$ where $f(f(z)) = z$).} on an extended state space.
...
\]
(`\texttt{=>}' is the Gen syntax for constructing hierarchical addresses).
