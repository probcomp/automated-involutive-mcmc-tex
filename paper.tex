...
% \left( \frac{d (\mu \circ f^{-1})}{d \mu} (x, y)\right)

Suppose the set of possible keys $\mathcal{A}$ can be partitioned $\mathcal{A} = \mathcal{I} \cup \mathcal{K}$ where is a set of \emph{discrete keys} $\mathcal{I}$ that use countable $V_a$ and counting measure $\mu_a$ for $a \in \mathcal{I}$ and $\mathcal{K}$ is a set of \emph{continuous keys} that use $V_a = \mathbb{R}^{d_a}$ for some $d_a$ and Lebesgue-measure $\mu_a$ for $a \in \mathcal{K}$.
%And suppose that there exists an involution $f : Z \to Z$ such that (i) 
Suppose there is a countable partition of $Z$ into $\{Z_e : e \in E\}$ such that across each $Z_e$ the set of keys $A$ and the values $\mathbf{x}_{\mathrm{d}}$ of discrete keys are both constant (so that each set $Z_e$ is isomorphic to a Euclidean space of assignments to the continuous keys).
