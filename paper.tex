...
The majority of these systems have focused on the automation of one or more built-in inference algorithms, with customization limited tuning parameters of the algorithm.
The programmable inference paradigm~\citep{mansinghka2018probabilistic} to probabilistic programming instead emphasizes user control over the inference algorithm and automation of the lower-level implementation.
...
\citet{roberts2019} presents a system embedded in Haskell that can automatically generate the implementation of certain reversible jump MCMC kernels from a high-level specification, but does not handle many kernels that can be handled by our technique, including the example in Figure~\ref{fig:structure-learning}.
...
\item The auxiliary probabilistic program them samples new values for those random choices (in this case, a new subtree).
