...
This section gives a measure-theoretic formulation of involutive MCMC on general state spaces.
This generality is necessary for applications of involutive MCMC to models with structure uncertainty and for describing how involutive MCMC can be automated using probabilistic programming.
...
\section{PROBABILISTIC PROGRAMMING WITH TRACES} \label{sec:prob-prog-traces}
...
The address $\mathtt{:}\mathtt{noise}$ has $V_a = \reals_{> 0}$ and $(\mathtt{"c"}, i)$ and $(\mathtt{"y"}, i)$ have $V_a = \reals$, and all of these use the Lebesgue measure. % TODO double-check this
The density with respect to the reference measure $\mu$ is:
\begin{align*}
p(x) =
\begin{array}{l}
