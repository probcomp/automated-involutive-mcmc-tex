...
By applying this rule recursively, we can instead compute the determinant of a much smaller matrix; for the example with $M=4$, the absolute value of the determinant simplifies to the absolute value of a single entry ($|-1|$).
Indeed, if those output variables that are the result of copying an input variable, then we can avoid computing those entries completely, and the computational complexity reduces from $M^2$ to $K^2$ where $K$ is the number of output variables that were copied from some input variable.
The $\mathtt{@copy}$ statement in our differentiable programming language makes the set of output variables that were copied immediately available to the interpreter.
Many involutive MCMC kernels update only a constant number of variables that does not depend on the size $N$ of the trace or the total number of continous keys $M$.
...
In this case, the size $N$ of the trace grows linearly in the number of clusters, and the number of data points, but the Jacobian determinant can be calculated from only the $6$-by-$6$ submatrix of the Jacobian that involves the parameters of the cluster being split (or the clusters being merged).
While a reasonable hand-coded implementation of this algorithm would likely exploit this sparsity as well, our differentiable programming languages for traces automates it.
