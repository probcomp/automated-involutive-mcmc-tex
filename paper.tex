...
We now show how to modify the program $\mathcal{F}$ and its interpreter, so that both computing $z' = (x', y')$ from $z = (x, y)$, and computing the log density ratio $\log(p(x') / p(x))$, can take $O(1)$ operations (that is, constant in $N_1 + N_1'$) when the involution $f$ has sparse structure and the model density $p$ has conditional independencies.
...
Then, the previous trace $x$ and a dictionary $\delta$ with $K_{\delta} = \Delta$ and $\delta[k] = x'[k]$ and the density function $p$ suffice to uniquely define the trace $x'$, provided the density satisfies the following condition, which is satisfied by densities represented as probabilistic programs:
...
Then, \textproc{naive-trace-update} (Algorithm~\ref{alg:naive-update}) computes the new trace $x'$ and log densities ratio $\log (p(x') / p(x))$ of a probabilistic program from a previous trace $x$ and a dictionary $\delta$, provided there exists $x' = \delta \oplus x|_A$ for some $A$ where $p(x') > 0$.
\begin{algorithm}[h]
\begin{algorithmic}
\Require probabilistic program $\mathcal{P}$; previous trace $x$;\\
dictionary $\delta$ such that there exists $x' = (\delta \oplus x|_{A})$ for some $A$ where $p(x') > 0$.
\vspace{1mm}
\Procedure{naive-trace-update}{$\mathcal{P}$, $x$, $\delta$}
   \State $x' \gets \{\}$
   \State $s' \gets 0$
   \execute[$\mathcal{P}$], but with $\mathtt{"}k \sim \mathrm{distribution}\mathtt{"} \equiv$ (
        \State 1. if $k \in K_{\delta}$ then $v \gets \delta[k]$ else $v \gets x[k]$
        \State 2. set $x'[k] \gets v$
        \State 3. set $s' \gets s' + \textproc{logpdf}(\mathrm{distribution}, v)$
        \State 4. evaluate to $v$)
    \END
   \State $s \gets \textproc{score}(\mathcal{P}, x)$
   \State \Return $(x', s' - s)$
\EndProcedure
\end{algorithmic}
...
\begin{algorithm}[h]
\begin{algorithmic}
\Procedure{involutive-mcmc-check}{$p$, $q$, $f$}
...
To test the user's algorithm, Gen first samples a model trace $x$ from the model prior $p$, and auxiliary trace $y$ from the $q_x$. It then runs three tests:
