...
It is possible to further reduce the complexity of applying an automated involutive MCMC kernel from $O(N + N')$ to $O(1)$ for many involutions, using additional optimizations that reduce the complexity of computing the density ratio and the complexity of computing the output of the involution to $O(1)$.

Consider input traces $z$ and output traces $z'$, with sizes $N$ and $N'$ respectively, and an involution $f$ such that $z' = f(z)$.
The approach in Algorithm~\ref{alg:auto-involutive-mcmc} involves running a program $\mathcal{F}$ that explicitly writes the value for each element of $f$, which requires $N'$ write operations.
We now show how to specify the involution $f$ such that computing $z'$ from $z$ can take less than $N'$ operations for involutions that possess sparse structure.
Recall that $K_{z}$ and $K_{z'}$ are the sets of keys in $z$ and $z'$, respectively, and that $z$ and $z'$ may both have values for certain keys, and $z$ may contains keys not present in $z'$, and vice-versa.
...
Suppose that $A \subseteq K_{z} \cap K_{z'}$ is the set of keys $k$ in both $z$ and $z'$ for which $z'[k] = z[k]$.
Let $\Delta := K_{z'} \setminus A$ be the set of keys in $z'$ that are either (i) not in $z$, or (ii) in $z$ but have their value changed.
Then, the previous trace $z$ and a dictionary $\delta$ with $K_{\delta} = \Delta$ and $\delta[k] = z'[k]$ and the density function $p$ suffice to uniquely define the trace $z'$, provided the density satisfies the following condition:
...
Therefore, the involution program can run in $O(|K_{\delta}|)$ operations.
...
When $|K_{\delta}|$ and $|K_{y}|$ and $|K_{y}$ are all constant in the size $N$ of the model trace $x$, the resulting algorithm scales as $O(1)$ instead of $O(N + N')$.
