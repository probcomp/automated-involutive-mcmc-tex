...
\item An extension to the algorithm that exploits sparsity in the involution to reduce the computational complexity of an involutive MCMC kernel from $O(N^3)$ to $O(1)$ in common cases, where $N$ is the dimensionality of the latent space.
...
Programmable inference~\citep{mansinghka2018probabilistic} proposes that inference algorithms be specified by users in high-level programming languages.
...
\citet{green1995reversible} uses a state space that is the countable union of `models', where each model uses a vector of real-valued parameters.
Dictionaries possess substantially more structure:
...
The probability is distributed among the finite sets of keys $K \subseteq \mathcal{K}$ where $|K| < \infty$:
\begin{equation}
...
For each $j \in \{0, 1, \ldots\}$, the probability for key set
...
Suppose there is an involution $g : E \to E$ between elements of the partition (a function with $g = g^{-1}$), and a family of continuously differentiable bijections $h_{e} : Z_e \to Z_{g(e)}$ indexed by $e \in E$, with $h_{e} = h_{g(e)}^{-1}$.
