...
\begin{algorithm}[h]
\begin{algorithmic}
\Procedure{simulate-and-score}{$P(\beta)$}
   \State $\log w \gets 0$
   \State $x \gets \{\}$
   \State Execute $P$ on input $\beta$, replacing $"\mathtt{a \sim d}"$ with:
    \State $\;\;\;\;$ 1. $\; v \sim d$
    \State $\;\;\;\;$ 2. $\; x[a] \gets v$
    \State $\;\;\;\;$ 3. $\; \log w \gets \log w + \textproc{logpdf}(d, x[a])$
    \State $\;\;\;\;$ 4. $\;$ \Return $v$
   \State \Return $(x, \log w)$
\EndProcedure

\Procedure{score}{$P(\beta), x$}
   \State $\log w \gets 0$
   \State Execute $Q$ on input $x$, replacing $"\mathtt{a \sim d}"$ with:
    \State $\;\;\;\;$ 1. $\; v \sim x[a]$
    \State $\;\;\;\;$ 3. $\; \log w \gets \log w + \textproc{logpdf}(d, x[a])$
    \State $\;\;\;\;$ 4. $\;$ \Return $v$
   \State \Return $\log w$
...
\Procedure{run-involution}{$F$, $(x, y)$}
    TODO
\EndProcedure
\end{algorithmic}
\caption{Probabilistic and Differentiable Program Operations}
\label{alg:prob-prog}
\end{algorithm}

\begin{algorithm}[h]
\begin{algorithmic}
\Procedure{auto-involutive-mcmc}{$P$, $Q$, $F$, $x$}
    \State $\log w_1 \gets \textproc{score}(P(), x)$
    \State $(y, \log w_2) \gets \textproc{simulate-and-score}(Q(x))$
    \State $(x', y', \log |J|) \gets \textproc{run-involution}(F, (x, y))$
    \State $\log w_2 \gets \textproc{score}(Q(x), y)$
    \State $\log w_3 \gets \textproc{score}(P(), x')$
    \State $\log w_4 \gets \textproc{score}(Q(x'), y')$
    \State $\log \alpha \gets \log w_3 - \log w_1 + \log w_4 - \log w_2 + \log |J|$
    \State $r \sim \mathrm{Uniform}(0, 1)$
    \State \algorithmicif \, $r \le \exp(\alpha)$ \algorithmicthen \, \Return $x'$ \algorithmicelse \, \Return $x$ 
\EndProcedure
\end{algorithmic}
...
\paragraph{Traces of Probabilistic Programs.}
...
Provided the two probabilistic programs $P$ and $Q$ satisfy a simple technical requirement (described in Section~\ref{sec:technical-requirement}), they can be used to specify the model density $p$ and auxiliary densities $p_x$, respectively for use with involutive MCMC on dictionaries (Algorithm~\ref{alg:involutive-mcmc}).
The acceptance probability of involutive MCMC includes the following ratio of densities:
\begin{equation}
\frac{p(x') q_{x'}(y')}{p(x) q_{x}(y)}
\end{equation}
To evaluate this density ratio, we can obtain four traces, one for each dictionary, and use those traces to compute the four probability densities, as shown in Algorithm~\ref{alg:trace-based-involutive-mcmc}.
Section~\ref{sec:sparsity} describes a more efficent approach that exploits sparsity in the involution and resulting cancellation in the ratio for improved efficiency, but requires a more sophisticated probabilistic programming runtime system.
